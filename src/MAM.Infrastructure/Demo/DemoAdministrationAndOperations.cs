...
public sealed class DemoAdministrationService(DemoSqliteDatabase database, IAuditSink audit) : IAdministrationService
{
    ...
    public async ValueTask DeleteUserAsync(Guid userId, string actorId, CancellationToken cancellationToken = default)
    {
        await using var c = await database.OpenAsync(cancellationToken);
        await using var q = c.CreateCommand();
        q.CommandText = "DELETE FROM DemoUser WHERE UserId=$id;";
        q.Parameters.AddWithValue("$id", userId.ToString("D"));
        ...
        await Audit(actorId, "administration.user.deleted", "MamUser", userId.ToString("D"), cancellationToken);
    }
