...
public sealed class DemoCurationService(DemoSqliteDatabase database, IAuditSink audit) : ICurationService
{
    ...
    public async ValueTask<TagSnapshot> UpdateTagAsync(Guid tagId,UpdateTagRequest request,string actorId,CancellationToken cancellationToken=default)
    {
        ...
        foreach(var change in changes){await using var up=c.CreateCommand();up.Transaction=tx;up.CommandText="UPDATE DemoAsset SET TagsJson=$tags WHERE AssetId=$id;";up.Parameters.AddWithValue("$tags",change.Json);up.Parameters.AddWithValue("$id",change.Id.ToString("D"));await up.ExecuteNonQueryAsync(cancellationToken);}
    ...
    public async ValueTask<TagDeletionResult> DeleteTagAsync(Guid tagId,long expectedVersion,bool removeFromAssets,string actorId,CancellationToken cancellationToken=default)
    {
        ...
        if(removeFromAssets){await using(var read=c.CreateCommand()){read.Transaction=tx;read.CommandText="SELECT AssetId,TagsJson FROM DemoAsset;";await using var rr=await read.ExecuteReaderAsync(cancellationToken);while(await rr.ReadAsync(cancellationToken)){var id=Guid.Parse(rr.GetString(0));var values=Tags(rr.GetString(1));var filtered=values.Where(x=>CurationTextNormalizer.NormalizeSearch(x)!=current.NormalizedName).ToArray();if(filtered.Length!=values.Count)changes.Add((id,JsonSerializer.Serialize(filtered)));}}foreach(var change in changes){await using var up=c.CreateCommand();up.Transaction=tx;up.CommandText="UPDATE DemoAsset SET TagsJson=$tags WHERE AssetId=$id;";up.Parameters.AddWithValue("$tags",change.Json);up.Parameters.AddWithValue("$id",change.Id.ToString("D"));await up.ExecuteNonQueryAsync(cancellationToken);}}
