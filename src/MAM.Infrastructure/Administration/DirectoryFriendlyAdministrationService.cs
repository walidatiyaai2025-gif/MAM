...
public sealed class DirectoryFriendlyAdministrationService : IAdministrationService
{
    ...
    public ValueTask DeleteUserAsync(Guid userId, string actorId, CancellationToken cancellationToken = default) => _inner.DeleteUserAsync(userId, actorId, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (roles.Any(role => !AllowedRoles.Contains(role)))
        {
            ...
            throw new AdministrationRequestException("invalid_role", "Only Administrator, CatalogEditor, Viewer, MediaDeleter, TapeManager, TapeOperator and TapeViewer roles are allowed.", 400);
        }
