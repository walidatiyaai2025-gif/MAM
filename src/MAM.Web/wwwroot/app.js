...
function shellPage(){
  const views={
    ...
    ingest:`${lead(arabic?'إدخال جديد':'New Ingest',arabic?'اختر مسار الإدخال المناسب.':'Choose an ingest path.')}<div class="grid two"><div class="card"><h3>${arabic?'إدارة الشرائط':'Tape Inventory'}</h3><p>${arabic?'إدارة سجلات الشرائط المادية فقط بدون تسجيل مباشر من الشريط داخل النظام.':'Manage physical tape records only. Direct tape recording is intentionally not part of MAM.'}</p><a class="action" href="/tape-inventory.html">${arabic?'فتح إدارة الشرائط':'Open tape management'}</a></div><div class="card"><h3>${arabic?'رفع الملفات':'Upload Files'}</h3><p>${arabic?'رفع ميديا واحدة أو مجلد كامل من نفس الشاشة.':'Upload one media file or a complete folder from the same workspace.'}</p><button class="action" data-go="upload">${arabic?'فتح مساحة الرفع':'Open upload workspace'}</button></div></div>`,
    ...
    admin:`${lead(arabic?'الإدارة':'Administration','P01 shell surface; authoritative identity and permissions are delivered through the Central API boundary.')}<div class="grid three"><div class="card metric"><strong>24</strong><span>Users</span><small>DEMO</small></div><div class="card metric"><strong>6</strong><span>Roles</span><small>DEMO</small></div><div class="card metric"><strong>1</strong><span>Tape inventory</span><small>CENTRAL API</small></div></div><div class="state denied"><strong>Permission denied</strong><br>This action requires the System Administrator role.</div>`,
