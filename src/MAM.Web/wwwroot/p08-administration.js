(() => {
  ...
  async function loadP08Administration() {
    ...
    content.innerHTML = `${lead(arabic ? 'إدارة المؤسسة والسياسات' : 'Enterprise Administration & Policy', arabic ? 'إدارة مركزية موثقة؛ لا يتم عرض كلمات المرور أو مفاتيح الخدمات.' : 'Authoritative central administration; secret material is never redisplayed.', 'P08 · CENTRAL API')}
      <div class="card">${state('loading', arabic ? 'جارٍ التحميل' : 'Loading', arabic ? 'جاري تحميل السياسات والمستخدمين والتدقيق…' : 'Loading policy, user and audit state…')}</div>`;
    try {
  ...
  function renderDashboard(overview, policies, users, audit, health) {
    const policyRows = (policies || []).map(p => `<tr>
      <td><strong>${esc(arabic && p.displayNameAr ? p.displayNameAr : p.displayNameEn)}</strong>${arabic && p.displayNameEn ? `<br><small>${esc(p.displayNameEn)}</small>` : (!arabic && p.displayNameAr ? `<br><small>${esc(p.displayNameAr)}</small>` : '')}</td>
      <td>${esc(p.category)}</td><td>v${Number(p.version || 0)}</td>
      <td>${p.requiresRestart ? `<span class="pill warning">${arabic ? 'إعادة تشغيل مطلوبة' : 'Restart'}</span>` : `<span class="pill">${arabic ? 'مباشر' : 'Live'}</span>`}</td>
    ...
    const userRows = (users || []).slice(0, 8).map(u => `<tr><td>${esc(u.displayName)}</td><td>${esc(u.userName)}</td><td>${esc((u.roles || []).join(', ') || '—')}</td><td>${u.isEnabled ? (arabic ? 'مفعّل' : 'Enabled') : (arabic ? 'معطّل' : 'Disabled')}</td><td>v${Number(u.version || 0)}</td></tr>`).join('');
    ...
    const healthCard = health.isReady
      ...
      : state('degraded', arabic ? 'حالة متدهورة' : 'Degraded', arabic ? 'مخزن سياسات الإدارة غير جاهز.' : (health.detail || 'Administration store is unavailable.'));
    ...
    content.innerHTML = `${lead(arabic ? 'إدارة المؤسسة والسياسات' : 'Enterprise Administration & Policy', arabic ? 'كل تعديل محمي بالصلاحيات، متزامن بإصدار، ومسجل في سجل التدقيق.' : 'Every mutation is permission-protected, versioned and audited.', 'P08 · LIVE')}
      <div class="grid four">
        <div class="card metric"><strong>${overview.policies ?? 0}</strong><span>${arabic ? 'السياسات' : 'Policies'}</span><small>${overview.enabledPolicies ?? 0} ${arabic ? 'مفعّلة' : 'enabled'}</small></div>
        <div class="card metric"><strong>${overview.users ?? 0}</strong><span>${arabic ? 'المستخدمون' : 'Users'}</span><small>${arabic ? 'الأدوار عبر واجهة API المركزية' : 'roles via Central API'}</small></div>
        <div class="card metric"><strong>${overview.dictionaryEntries ?? 0}</strong><span>${arabic ? 'القواميس' : 'Dictionary entries'}</span><small>${arabic ? 'بيانات وصفية ثنائية اللغة' : 'bilingual metadata'}</small></div>
        <div class="card metric"><strong>${overview.restartRequired ?? 0}</strong><span>${arabic ? 'يتطلب إعادة تشغيل' : 'Restart-impact'}</span><small>${arabic ? 'أثر تشغيلي صريح' : 'explicit operational impact'}</small></div>
      </div>
      ...
      <div class="card"><h3>${arabic ? 'السياسات المركزية' : 'Authoritative policies'}</h3><div class="table-wrap"><table><thead><tr><th>${arabic ? 'السياسة' : 'Policy'}</th><th>${arabic ? 'الفئة' : 'Category'}</th><th>${arabic ? 'الإصدار' : 'Version'}</th><th>${arabic ? 'الأثر' : 'Impact'}</th><th>SecretRef</th><th></th></tr></thead><tbody>${policyRows || `<tr><td colspan="6">${arabic ? 'لا توجد سياسات.' : 'No policies.'}</td></tr>`}</tbody></table></div><div id="p08Editor"></div></div>
      <div class="card"><h3>${arabic ? 'المستخدمون والأدوار' : 'Users & roles'}</h3><div class="table-wrap"><table><thead><tr><th>${arabic ? 'الاسم' : 'Display name'}</th><th>${arabic ? 'المستخدم' : 'User'}</th><th>${arabic ? 'الأدوار' : 'Roles'}</th><th>${arabic ? 'الحالة' : 'Status'}</th><th>${arabic ? 'الإصدار' : 'Version'}</th></tr></thead><tbody>${userRows || `<tr><td colspan="5">${arabic ? 'لا توجد سجلات مستخدمين بعد.' : 'No user policy records yet.'}</td></tr>`}</tbody></table></div><p><small>${arabic ? 'توفير هوية الإنتاج نفسه يظل من اختصاص موفر الهوية في P12؛ P08 يدير سياسة الصلاحيات فقط.' : 'Production identity provisioning remains external/P12; P08 governs MAM authorization policy only.'}</small></p></div>
      <div class="card"><div class="toolbar"><h3>${arabic ? 'سجل التدقيق' : 'Audit explorer'}</h3><button id="p08Export" class="action">${arabic ? 'تصدير CSV' : 'Export CSV'}</button></div><div class="table-wrap"><table><thead><tr><th>UTC</th><th>${arabic ? 'المنفّذ' : 'Actor'}</th><th>${arabic ? 'الإجراء' : 'Action'}</th><th>${arabic ? 'النتيجة' : 'Outcome'}</th></tr></thead><tbody>${auditRows || `<tr><td colspan="4">${arabic ? 'لا توجد أحداث تدقيق.' : 'No audit events.'}</td></tr>`}</tbody></table></div><div id="p08AuditState" aria-live="polite"></div></div>`;
  ...
  function openPolicyEditor(key, policies) {
    ...
    host.innerHTML = `<div class="card" style="margin-top:16px"><h3>${esc(arabic && policy.displayNameAr ? policy.displayNameAr : policy.displayNameEn)}</h3>
      ...
      <label><input id="p08Enabled" type="checkbox" ${policy.isEnabled ? 'checked' : ''}/> ${arabic ? 'مفعّلة' : 'Enabled'}</label>
    ...
    document.getElementById('p08Validate').addEventListener('click', async () => {
      status.innerHTML = state('loading', arabic ? 'جارٍ التحميل' : 'Loading', arabic ? 'جاري التحقق…' : 'Validating policy…');
      try {
        ...
        status.innerHTML = result.valid
          ? state('empty', arabic ? 'صالح' : 'Valid', arabic ? 'السياسة صالحة للحفظ.' : 'Policy is valid for persistence.')
          : state('error', arabic ? 'مرفوض' : 'Rejected', arabic ? 'السياسة مرفوضة. راجع القيم ثم أعد التحقق.' : (result.errors || []).join(' '));
      } catch (e) { showEditError(status, e); }
    });
    document.getElementById('p08Test').addEventListener('click', async () => {
      status.innerHTML = state('loading', arabic ? 'جارٍ التحميل' : 'Loading', arabic ? 'اختبار مرجع الخادم بدون إظهار السر…' : 'Testing server-side reference without exposing secret material…');
      try {
        const result = await adminRequest(`/client-api/admin/policies/${encodeURIComponent(key)}/test`, { method: 'POST' });
        const detail = arabic ? (result.success ? 'نجح اختبار مرجع الخادم.' : 'فشل اختبار مرجع الخادم.') : result.detail;
        status.innerHTML = result.success ? state('empty', result.code, detail) : state('degraded', result.code, detail);
      } catch (e) { showEditError(status, e); }
    });
    document.getElementById('p08Save').addEventListener('click', async () => {
      status.innerHTML = state('loading', arabic ? 'جارٍ التحميل' : 'Loading', arabic ? 'جاري الحفظ بإصدار متفائل…' : 'Saving with optimistic concurrency…');
      try {
        const saved = await adminRequest(`/client-api/admin/policies/${encodeURIComponent(key)}`, { method: 'PUT', body: JSON.stringify(body()) });
        status.innerHTML = state('empty', arabic ? 'تم الحفظ' : 'Saved', arabic ? `الإصدار ${saved.version}${saved.requiresRestart ? ' · إعادة تشغيل مطلوبة' : ''}` : `Version ${saved.version}${saved.requiresRestart ? ' · restart required' : ''}`);
  ...
  function showEditError(host, error) {
    if (error.kind === 'conflict') host.innerHTML = state('degraded', arabic ? 'تعارض' : 'Conflict', arabic ? 'تم تعديل السياسة في جلسة أخرى. أعد التحميل قبل الحفظ.' : 'The policy changed in another session. Refresh before saving.');
    else if (error.kind === 'invalid_json') host.innerHTML = state('error', arabic ? 'JSON غير صالح' : 'Invalid JSON', arabic ? 'صيغة JSON غير صحيحة.' : 'Policy JSON is invalid.');
    else if (error.kind === 'denied') host.innerHTML = state('denied', arabic ? 'الوصول مرفوض' : 'Permission denied', arabic ? 'لا توجد صلاحية إدارة.' : 'Administration permission is required.');
    else host.innerHTML = state(error.kind === 'degraded' ? 'degraded' : 'error', arabic ? (error.kind === 'degraded' ? 'حالة متدهورة' : 'خطأ في واجهة API') : (error.kind === 'degraded' ? 'Degraded' : 'API error'), arabic ? 'تعذر إتمام العملية.' : (error.message || 'Operation failed.'));
  }

  async function exportAudit() {
    const host = document.getElementById('p08AuditState');
    host.innerHTML = state('loading', arabic ? 'جارٍ التحميل' : 'Loading', arabic ? 'جاري تجهيز CSV…' : 'Preparing audit CSV…');
    try {
      ...
      host.innerHTML = state('empty', arabic ? 'تم التصدير' : 'Exported', arabic ? 'تم تجهيز سجل التدقيق.' : 'Audit CSV prepared.');
    } catch (e) { host.innerHTML = state(e.kind === 'denied' ? 'denied' : 'error', arabic ? (e.kind === 'denied' ? 'الوصول مرفوض' : 'خطأ في واجهة API') : (e.kind === 'denied' ? 'Permission denied' : 'API error'), arabic ? 'تعذر تصدير سجل التدقيق.' : 'Audit export failed.'); }
  }
