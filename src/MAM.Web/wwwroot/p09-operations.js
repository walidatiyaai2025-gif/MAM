    ...
    content.innerHTML=`<div class="p132-report">
      ...
      <section class="p132-report-panel" data-report-panel="protection" hidden>
        <div class="p132-report-grid">
          ...
          <article class="p132-report-card"><h3>${arabic?'البايتات التي تم التحقق منها':'Verified bytes'}</h3><p><strong>${bytes(integrity.protectedBytes)}</strong></p><p><small>${arabic?'لا يتم إرسال مسارات الملفات أو بيانات الاعتماد إلى المتصفح.':'Filesystem roots and credentials are never sent to the browser.'}</small></p></article>
        </div>
      </section>
