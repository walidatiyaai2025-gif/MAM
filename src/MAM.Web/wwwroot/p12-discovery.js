...
async function p12LoadReferences(){
  ...
  try{
    p12ReferenceCache=await p12Json('/client-api/discovery/references');if(route!=='references')return;
    host.innerHTML=`<div class="card"><h3>${arabic?'إضافة مرجع جديد':'Add reference subject'}</h3>
      <div class="grid two"><div><label>${arabic?'الاسم بالإنجليزية':'English name'}</label><input id="p12RefEn" maxlength="200" placeholder="Name (English)"/></div><div><label>${arabic?'الاسم بالعربية':'Arabic name'}</label><input id="p12RefAr" maxlength="200" placeholder="الاسم بالعربية" dir="rtl"/></div></div>
      <div class="grid two"><div><label>${arabic?'الوصف بالإنجليزية':'English description'}</label><textarea id="p12RefDescriptionEn" maxlength="1000" rows="3"></textarea></div><div><label>${arabic?'الوصف بالعربية':'Arabic description'}</label><textarea id="p12RefDescriptionAr" maxlength="1000" rows="3" dir="rtl"></textarea></div></div>
      <div class="toolbar"><input id="p12RefTags" maxlength="1000" placeholder="${arabic?'وسوم مفصولة بفاصلة':'Comma-separated tags'}"/><button id="p12RefCreate" class="action"><i class="bi bi-plus-circle"></i> ${arabic?'إضافة المرجع':'Add reference'}</button></div><div id="p12RefState" aria-live="polite"></div></div>
      <div class="grid three">${p12ReferenceCache.map(r=>`<div class="card"><div class="toolbar"><h3>${esc(arabic&&r.nameAr?r.nameAr:r.nameEn)}</h3><span class="badge">${r.isActive?(arabic?'نشط':'Active'):(arabic?'معطل':'Inactive')}</span></div><p>${esc(arabic?(r.descriptionAr||r.descriptionEn||''):(r.descriptionEn||r.descriptionAr||''))}</p><p>${esc((r.tags||[]).join(' · '))}</p><p>${arabic?'صور مرجعية':'Reference images'}: ${esc((r.referenceAssetIds||[]).length)}</p><div class="toolbar"><button class="action" data-p12-ref-edit="${esc(r.subjectId)}"><i class="bi bi-pencil-square"></i> ${arabic?'تعديل':'Edit'}</button><button class="action p127-danger" data-p12-ref-delete="${esc(r.subjectId)}"><i class="bi bi-trash3"></i> ${arabic?'حذف':'Delete'}</button></div><div class="toolbar"><input data-p12-ref-image-input="${esc(r.subjectId)}" placeholder="Image Asset ID"/><button class="action" data-p12-ref-image-add="${esc(r.subjectId)}">${arabic?'ربط صورة':'Attach image'}</button></div><small>${(r.referenceAssetIds||[]).map(x=>esc(x)).join('<br>')}</small></div>`).join('')}</div>`;
    document.getElementById('p12RefCreate')?.addEventListener('click',()=>void p12CreateReference());
    host.querySelectorAll('[data-p12-ref-edit]').forEach(button=>button.addEventListener('click',()=>void p12EditReference(button.dataset.p12RefEdit)));
    host.querySelectorAll('[data-p12-ref-delete]').forEach(button=>button.addEventListener('click',()=>void p12DeleteReference(button.dataset.p12RefDelete)));
    host.querySelectorAll('[data-p12-ref-image-add]').forEach(button=>button.addEventListener('click',()=>void p12AddReferenceImage(button.dataset.p12RefImageAdd)));
  }catch(ex){host.innerHTML=p12Failure(ex,arabic?'تعذر تحميل مكتبة المراجع.':'Reference library could not be loaded.');}
}
async function p12CreateReference(){
  const output=document.getElementById('p12RefState');const nameEn=document.getElementById('p12RefEn')?.value.trim()||'';
  if(!nameEn){if(output)output.innerHTML=state('error','Validation',arabic?'الاسم الإنجليزي مطلوب.':'English name is required.');return;}
  const nameAr=document.getElementById('p12RefAr')?.value.trim()||'';
  const descriptionEn=document.getElementById('p12RefDescriptionEn')?.value.trim()||'';
  const descriptionAr=document.getElementById('p12RefDescriptionAr')?.value.trim()||'';
  const tags=(document.getElementById('p12RefTags')?.value||'').split(',').map(x=>x.trim()).filter(Boolean);
  try{await p12Json('/client-api/discovery/references',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nameEn,nameAr:nameAr||null,descriptionEn:descriptionEn||null,descriptionAr:descriptionAr||null,tags})});window.MamPopup?.notify(arabic?'تمت إضافة المرجع.':'Reference added.','success');await p12LoadReferences();}catch(ex){if(output)output.innerHTML=p12Failure(ex,arabic?'تعذر إضافة المرجع.':'Reference subject could not be created.');}
}
async function p12EditReference(subjectId){
  const subject=p12ReferenceCache.find(x=>String(x.subjectId)===String(subjectId));if(!subject)return;
  const modal=await window.p127OpenModal?.({title:arabic?'تعديل المرجع':'Edit reference',confirmText:arabic?'حفظ':'Save',body:`<div class="p127-user-form"><div class="p127-field"><label>English</label><input id="p12EditRefEn" maxlength="200" value="${esc(subject.nameEn||'')}"/></div><div class="p127-field"><label>العربية</label><input id="p12EditRefAr" maxlength="200" dir="rtl" value="${esc(subject.nameAr||'')}"/></div><div class="p127-field full"><label>${arabic?'الوصف بالإنجليزية':'English description'}</label><textarea id="p12EditRefDescriptionEn" maxlength="1000" rows="3">${esc(subject.descriptionEn||'')}</textarea></div><div class="p127-field full"><label>${arabic?'الوصف بالعربية':'Arabic description'}</label><textarea id="p12EditRefDescriptionAr" maxlength="1000" rows="3" dir="rtl">${esc(subject.descriptionAr||'')}</textarea></div><div class="p127-field full"><label>${arabic?'الوسوم':'Tags'}</label><input id="p12EditRefTags" maxlength="1000" value="${esc((subject.tags||[]).join(', '))}"/></div><div class="p127-field full"><label><input id="p12EditRefActive" type="checkbox" ${subject.isActive?'checked':''}/> ${arabic?'نشط':'Active'}</label></div></div>`});
  if(!modal)return;
  const body={nameEn:modal.querySelector('#p12EditRefEn').value.trim(),nameAr:modal.querySelector('#p12EditRefAr').value.trim()||null,descriptionEn:modal.querySelector('#p12EditRefDescriptionEn').value.trim()||null,descriptionAr:modal.querySelector('#p12EditRefDescriptionAr').value.trim()||null,tags:modal.querySelector('#p12EditRefTags').value.split(',').map(x=>x.trim()).filter(Boolean),isActive:modal.querySelector('#p12EditRefActive').checked};
  if(!body.nameEn){window.MamPopup?.notify(arabic?'الاسم الإنجليزي مطلوب.':'English name is required.','error');return;}
  try{await p12Json(`/client-api/discovery/references/${subjectId}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});window.MamPopup?.notify(arabic?'تم تعديل المرجع.':'Reference updated.','success');await p12LoadReferences();}catch(ex){const output=document.getElementById('p12RefState');if(output)output.innerHTML=p12Failure(ex,arabic?'تعذر تعديل المرجع.':'Reference could not be updated.');}
}
async function p12DeleteReference(subjectId){
  const subject=p12ReferenceCache.find(x=>String(x.subjectId)===String(subjectId));if(!subject)return;
  const modal=await window.p127OpenModal?.({title:arabic?'حذف المرجع':'Delete reference',danger:true,confirmText:arabic?'حذف نهائي':'Delete permanently',body:`<p>${arabic?'سيتم حذف المرجع وروابطه مع الوسائط.':'The reference and its media tag links will be deleted.'}</p><strong>${esc(arabic&&subject.nameAr?subject.nameAr:subject.nameEn)}</strong>`});
  if(!modal)return;
  try{await p12Json(`/client-api/discovery/references/${subjectId}`,{method:'DELETE'});window.MamPopup?.notify(arabic?'تم حذف المرجع.':'Reference deleted.','success');await p12LoadReferences();}catch(ex){const output=document.getElementById('p12RefState');if(output)output.innerHTML=p12Failure(ex,arabic?'تعذر حذف المرجع.':'Reference could not be deleted.');}
}
