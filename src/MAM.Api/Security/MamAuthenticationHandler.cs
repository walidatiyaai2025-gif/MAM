...
public sealed class MamAuthenticationHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    ...
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        if (string.Equals(_settings.Auth.Mode, "ActiveDirectory", StringComparison.OrdinalIgnoreCase))
        {
            return await HandleActiveDirectoryAsync(Context.RequestAborted);
        }

        return AuthenticateResult.NoResult();
    }

    private AuthenticateResult HandleDevelopment()
    {
        ...
        if (identity is null) return AuthenticateResult.Fail("Unknown development identity.");
        return Success(identity);
    }

    private async Task<AuthenticateResult> HandleActiveDirectoryAsync(CancellationToken cancellationToken)
    {
        if (!Request.Headers.TryGetValue(ProductionUserHeader, out var userHeader) ||
            !Request.Headers.TryGetValue(ProductionTimestampHeader, out var timestampHeader) ||
            !Request.Headers.TryGetValue(ProductionSignatureHeader, out var signatureHeader))
        {
            return AuthenticateResult.NoResult();
        }
        ...
        if (string.IsNullOrWhiteSpace(userName) || userName.Length > 200)
            return AuthenticateResult.Fail("Invalid production identity.");

        if (!long.TryParse(timestampHeader.ToString(), NumberStyles.None, CultureInfo.InvariantCulture, out var unixSeconds))
            return AuthenticateResult.Fail("Invalid production identity timestamp.");

        DateTimeOffset issuedAt;
        try { issuedAt = DateTimeOffset.FromUnixTimeSeconds(unixSeconds); }
        catch (ArgumentOutOfRangeException) { return AuthenticateResult.Fail("Invalid production identity timestamp."); }

        if ((DateTimeOffset.UtcNow - issuedAt).Duration() > SignatureLifetime)
            return AuthenticateResult.Fail("Expired production identity signature.");

        var encodedKey = Environment.GetEnvironmentVariable("MAM_INTERNAL_AUTH_KEY");
        if (string.IsNullOrWhiteSpace(encodedKey))
            return AuthenticateResult.Fail("Production identity authority is unavailable.");
        ...
        try
        {
            ...
            supplied = Convert.FromBase64String(signatureHeader.ToString());
        }
        catch (FormatException)
        {
            return AuthenticateResult.Fail("Invalid production identity signature.");
        }
        ...
        if (key.Length < 32)
        {
            ...
            return AuthenticateResult.Fail("Production identity authority is invalid.");
        }
        ...
        if (string.IsNullOrWhiteSpace(connectionString))
            return AuthenticateResult.Fail("Authoritative user store is unavailable.");

        try
        {
            await using var connection = new SqlConnection(connectionString);
            await connection.OpenAsync(cancellationToken);
            var user = await ReadUserAsync(connection, userName, cancellationToken);

            if (user is null && IsBootstrapAdministrator(userName))
            {
                await ProvisionBootstrapAdministratorAsync(connection, userName, cancellationToken);
                user = await ReadUserAsync(connection, userName, cancellationToken);
            }
        ...
        {
            Logger.LogError(ex, "Production identity lookup failed for {UserName}.", userName);
            return AuthenticateResult.Fail("Authoritative user store is unavailable.");
        }
    }

    private bool IsBootstrapAdministrator(string userName) =>
        _settings.Auth.BootstrapAdministrators.Any(item => string.Equals(item?.Trim(), userName, StringComparison.OrdinalIgnoreCase));
    ...
    private static async Task<ResolvedUser?> ReadUserAsync(SqlConnection connection, string userName, CancellationToken cancellationToken)
    {
        const string sql = """
            ...
            WHERE u.UserName=@UserName OR u.ExternalSubject=@UserName
            ORDER BY r.RoleName;
            """;
    ...
    private static async Task ProvisionBootstrapAdministratorAsync(SqlConnection connection, string userName, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            const string grant = """
                ...
                FROM dbo.MamUser u CROSS JOIN dbo.MamRole r
                WHERE (u.UserName=@UserName OR u.ExternalSubject=@UserName)
    ...
    private sealed class ResolvedUserBuilder
    {
        public ResolvedUserBuilder(Guid userId, string userName, string displayName, bool isEnabled)
        {
            UserId = userId;
            UserName = userName;
        ...
        public ResolvedUser Build() => new(UserId, UserName, DisplayName, IsEnabled, Roles.ToArray());
    }
}
