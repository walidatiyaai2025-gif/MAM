...
At runtime the Production Desktop ignores stale machine-level Demo/development API overrides and clears `MAM_DEV_USER`. Production authentication supports both Windows SSO and explicit Active Directory credential sign-in through the Web `/auth/ad` session authority. A Windows workstation therefore does not need to be domain-joined: a user can enter `DA\\username` and password in the Desktop login screen, and the password is used only for that sign-in request and is not stored. Windows SSO is now strictly a one-time, explicitly selected bootstrap option. The Web application's default authentication scheme is the MAM session cookie; absence of a cookie never triggers Negotiate automatically. **All routine Desktop API traffic is cookie-only with `UseDefaultCredentials=false` and redirects disabled**, so a Workgroup PC can never silently fall back into Kerberos/NTLM while calling `/client-api`. If the application session expires, the Desktop invalidates the local session and returns to sign-in instead of misreporting Central API health as degraded. Production session tickets use a 12-hour sliding lifetime. IIS/ARR forwarded client/scheme headers are trusted only from the local reverse proxy so HTTPS origin checks and AD-login rate limiting see the actual workstation instead of every request appearing as loopback.
...
### P12.21 navigation and Media Library ownership hardening
