...
try{
 ...
 var nav=await Eval(socket,++id,"(async()=>{const b=[...document.querySelectorAll('#nav [data-route=\"reports\"]')].find(x=>!x.hidden);if(!b)return{ok:false};b.click();await new Promise(r=>setTimeout(r,40));const staleButton=[...document.querySelectorAll('#nav [data-route=\"library\"]')].find(x=>!x.hidden);if(staleButton)staleButton.click();const afterSynthetic=new URLSearchParams(location.hash.slice(1)).get('route');const stale=new URL(location.href),staleHash=new URLSearchParams(stale.hash.slice(1));staleHash.set('route','library');stale.hash=staleHash.toString();history.replaceState(history.state,'',stale.href);const immediate=new URLSearchParams(location.hash.slice(1)).get('route');await new Promise(r=>setTimeout(r,100));const h=new URLSearchParams(location.hash.slice(1));return{ok:true,afterSynthetic,immediate,r:h.get('route'),t:document.querySelector('#pageTitle')?.textContent?.trim()||'',lang:document.documentElement.lang,dir:document.documentElement.dir,authority:window.mamRouteAuthority?.diagnose?.()||null}})()",true);
 if(!nav.GetProperty("ok").GetBoolean()||S(nav,"afterSynthetic")!="reports"||S(nav,"immediate")!="reports"||S(nav,"r")!="reports"||S(nav,"t")!="التقارير"||S(nav,"lang")!="ar"||S(nav,"dir")!="rtl") throw new Exception($"Arabic navigation failed: {nav.GetRawText()}");
