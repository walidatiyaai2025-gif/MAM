...
static async Task<int> Settle(ClientWebSocket s,int id){await Eval(s,++id,"new Promise(r=>{const d=()=>requestAnimationFrame(()=>requestAnimationFrame(()=>setTimeout(r,500)));document.readyState==='complete'?d():addEventListener('load',d,{once:true})})",true);return id;}
static async Task<JsonElement> Eval(ClientWebSocket s,int id,string e,bool ap=false)=>await Call(s,id,"Runtime.evaluate",new{expression=e,awaitPromise=ap,returnByValue=true}).ContinueWith(t=>t.Result.GetProperty("result").GetProperty("value"));
