...
try{
 ...
 var preserved=await Eval(socket,++id,"(()=>{const stale=new URL(location.href);stale.hash='route=library';history.replaceState(history.state,'',stale.href);const h=new URLSearchParams(location.hash.slice(1));return{route:h.get('route'),q:h.get('q'),page:h.get('page'),view:h.get('view'),sort:h.get('sort'),authority:window.mamRouteAuthority?.diagnose?.()||null}})()");
 if(S(preserved,"route")!="library"||S(preserved,"q")!="Diwan"||S(preserved,"page")!="2"||S(preserved,"view")!="list"||S(preserved,"sort")!="title") throw new Exception($"Bootstrap deep-link authority failed: {preserved.GetRawText()}");
 ...
 var nav=await Eval(socket,++id,"(async()=>{const b=[...document.querySelectorAll('#nav [data-route=\"reports\"]')].find(x=>!x.hidden);if(!b)return{ok:false};b.click();await new Promise(r=>setTimeout(r,80));const stale=new URL(location.href),staleHash=new URLSearchParams(stale.hash.slice(1));staleHash.set('route','library');stale.hash=staleHash.toString();history.replaceState(history.state,'',stale.href);const immediate=new URLSearchParams(location.hash.slice(1)).get('route');await new Promise(r=>setTimeout(r,100));const h=new URLSearchParams(location.hash.slice(1));return{ok:true,immediate,r:h.get('route'),t:document.querySelector('#pageTitle')?.textContent?.trim()||'',lang:document.documentElement.lang,dir:document.documentElement.dir,writerOwner:history.replaceState?.__mamRouteAuthorityOwner||'',authority:window.mamRouteAuthority?.diagnose?.()||null}})()",true);
 if(!nav.GetProperty("ok").GetBoolean()||S(nav,"immediate")!="reports"||S(nav,"r")!="reports"||S(nav,"t")!="التقارير"||S(nav,"lang")!="ar"||S(nav,"dir")!="rtl"||S(nav,"writerOwner")!="p131-route-authority-10") throw new Exception($"Arabic navigation failed: {nav.GetRawText()}");
...
static async Task<int> WaitPort(string p,Process b){var f=Path.Combine(p,"DevToolsActivePort");for(var i=0;i<450;i++){if(File.Exists(f)){try{using var fs=new FileStream(f,FileMode.Open,FileAccess.Read,FileShare.ReadWrite|FileShare.Delete);using var sr=new StreamReader(fs);var first=await sr.ReadLineAsync();if(int.TryParse(first,out var n))return n;}catch(IOException){}catch(UnauthorizedAccessException){}}if(b.HasExited)throw new Exception("Chromium exited");await Task.Delay(100);}throw new Exception("DevTools timeout");}
