...
param(
    ...
    [string]$CaptureCardModel = '',
    [string]$TapeDeckModel = '',
    [string]$InputName = '',
    [string]$Container = '',
    [string]$Codec = '',
...
if (-not $PreviewVerified) { Add-Failure $failures 'Real-device live preview has not been verified.' }
if (-not $AudioMetersVerified) { Add-Failure $failures 'Real-device audio meters/channel state have not been verified.' }
if (-not $TimecodeVerified) { Add-Failure $failures 'Real-device timecode behavior has not been verified.' }
if (-not $CentralApiUploadVerified) { Add-Failure $failures 'Central API durable upload has not been verified.' }
if (-not $BackupProtectedVerified) { Add-Failure $failures 'P06 verified Backup protection has not been verified.' }
if (-not $RestartRecoveryVerified) { Add-Failure $failures 'Restart/interruption recovery has not been verified.' }
if (-not $NetworkRecoveryVerified) { Add-Failure $failures 'Network-loss/retry recovery has not been verified.' }
if (-not $CacheCleanupVerified) { Add-Failure $failures 'Safe temporary-cache cleanup has not been verified.' }
if (-not $OwnerSiteAccepted) { Add-Failure $failures 'Owner/site acceptance has not been explicitly recorded.' }
